...
def mmd_loss2(Xs, Xt, kernel_function):
    """
    Compute the Maximum Mean Discrepancy (MMD) loss.

    Parameters:
    - Xs: Tensor representing source domain features.
    - Xt: Tensor representing target domain features.
    - kernel_function: Kernel function.

    Returns:
    - mmd_loss: Computed MMD loss.
    """

    # Compute pairwise kernel evaluations
    Kss = kernel_function(Xs, Xs)
    Kst = kernel_function(Xs, Xt)
    Ktt = kernel_function(Xt, Xt)

    # Compute MMD loss
    n_s = Xs.size(0)
    n_t = Xt.size(0)

    mmd_loss = (1.0 / (n_s * n_s)) * torch.sum(Kss) - \
                (2.0 / (n_s * n_t)) * torch.sum(Kst) + \
                (1.0 / (n_t * n_t)) * torch.sum(Ktt)
